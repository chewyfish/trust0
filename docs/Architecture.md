...
|---------------------|------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Proxy Connections   | `T0C` <--> `T0G`<br> `T0G (cli)` <--> `T0G (svc)` | <p>This event is sent bidirectionally every `6 secs`. It contains the current list of service<br>proxy connections' bind address pairs (as known by the gateway (1)) for the<br>respective user session.</p><p>Each side will keep track of missing binds as a consecutive count. When that reaches `5`,<br>those corresponding missing connection(s) will be shut down.</p><p>Additionally, each side will also keep track of consecutive missing `Proxy Connections`<br>signal events. when that reaches `5`, the entire user session(control plane, service<br>proxy connections) will be shut down.</p><p>(1) - Upon TLS connection establishment an initial message, detailing the connection bind<br>addresses, is sent from the gateway to the client (and the service-gateway to the client-gateway for DMZ setup). This address pair will be used in<br>coordinating the active state for the respective connection. |
...
To recap, 3 or 4 total connections make up the virtual connection to a service, whether DMZ topology is used or not:

&nbsp;&nbsp;&nbsp;&nbsp;<u>Service Client</u> ` <--TCP|UDP|TLS--> ` <u>T0C</u> ` <--mTLS--> ` <u>T0G</u> ` <--TCP|UDP|TLS--> ` <u>Service</u>.

&nbsp;&nbsp;&nbsp;&nbsp;<u>Service Client</u> ` <--TCP|UDP|TLS--> ` <u>T0C</u> ` <--mTLS--> ` <u>T0G</u> ` <--mTLS--> ` <u>T0G</t> ` <--TCP|UDP|TLS--> ` <u>Service</u>.
