...
PS1='$ ' "${TMUX_CMD}" new-session -s trust0-ssh-tcp-example \; \
  set -g mouse on \; \
  send-keys '(clear && read -p "Step 1: Ensure SSHD is listening on '"${SSHD_SERVICE__PORT}"'. Hit <Enter> to go to shell prompt if needed.")' C-m \; \
  split-window -v -l 80% \; \
  ...
  split-window -v -l 70% \; \
  send-keys '(clear && echo "Step 3.1: Hit <Enter> to run trust0 client (after gateway is up)" && read -p "Step 3.2: Enter \"start -s sshd-tcp -p '"${SSHD_PROXY__PORT}"'\" (to start service proxy)" && "'"${GMAKE_CMD}"'" run-trust0-client-nodeps EXECBIN_EXTRA_ARGS="'"${CLIENT_BIN_ARGS}"'")' C-m \; \
  split-window -v -l 60% \; \
  send-keys '(clear && read -p "Step 4: Hit <Enter> to run ssh session" && echo "\"'"${SSH_CMD}"'\" -p '"${SSHD_PROXY__PORT}"' \"'"${USER}@localhost"'\"" && "'"${SSH_CMD}"'" -p '"${SSHD_PROXY__PORT}"' "'"${USER}@localhost"'" )' C-m \; \
  select-pane -t 0 \; \
  split-window -h -l 25% \; \
  send-keys '(clear && read -p "Step Last: Hit <Enter> to shutdown example" && ("'"${TMUX_CMD}"'" kill-session -t trust0-ssh-tcp-example))' C-m \; \
  select-pane -t 0 \;
