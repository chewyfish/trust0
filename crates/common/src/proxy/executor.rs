    ...
    pub fn poll_new_tasks(&mut self) -> Result<(), AppError> {
        ...
        loop {
            ...
            let task = self.proxy_tasks_receiver.recv().map_err(|err|
                AppError::GenWithMsgAndErr("Error receiving new event task".to_string(), Box::new(err)))?;
            ...
            match task {
                ...
                ProxyExecutorEvent::OpenChannelAndTcpProxy(proxy_key, proxy_context) => {
                    ...
                    match ChannelAndTcpStreamProxy::new(&proxy_key,
                                                        proxy_context.0,
                                                        proxy_context.1,
                                                        proxy_context.2,
                                                        proxy_context.3,
                        ...
                        Ok(proxy_stream) => {
                            ...
                            if let Err(err) = proxy_stream.lock().unwrap().connect() {
                                error(&target!(), &format!("Error connecting proxy streams: proxy_stream={}, err={:?}", &proxy_key, err));
                                continue;
                            }
                        ...
                        Err(err) => {
                            ...
                            if let Err(err) = proxy_channel_sender.send(ProxyEvent::Closed(proxy_key.clone())) {
                ...
                ProxyExecutorEvent::OpenTcpAndTcpProxy(proxy_key, proxy_context) => {
                    ...
                    match TcpAndTcpStreamProxy::new(&proxy_key,
                                                    proxy_context.0,
                                                    proxy_context.1,
                                                    proxy_context.2,
                                                    proxy_context.3,
                ...
                ProxyExecutorEvent::OpenTcpAndUdpProxy(proxy_key, proxy_context) => {
                    ...
                    match TcpAndUdpStreamProxy::new(&proxy_key,
                                                    proxy_context.0,
                                                    proxy_context.1,
                                                    proxy_context.2,
                                                    proxy_context.3) {
                ...
                ProxyExecutorEvent::Close(proxy_key) => {
                    ...
                    match self.proxy_streams.get_mut(&proxy_key) {
                        ...
                        None => {
                            warn(&target!(), &format!("Unknown proxy for closure: proxy_stream={}", &proxy_key));
                            continue;
                        }
                    }
                }
