...
impl ChannelAndTcpStreamProxy {
    ...
    pub fn new(proxy_key: &str,
        ...
        tcp_stream.set_nonblocking(true).map_err(|err|
            AppError::GenWithMsgAndErr(format!("Failed making tcp stream non-blocking: stream_addr={}", &proxy_key),Box::new(err)))?;
    ...
    pub fn connect(&mut self) -> Result<(), AppError> {
        ...
        *self.closing.lock().unwrap() = false;
        ...
        let channel_to_tcp_iocopy_handle = {
            ...
            let tcp_stream = mio::net::TcpStream::from_std(stream_utils::clone_std_tcp_stream(&self.tcp_stream)?);
        ...
        thread::spawn(move || {
            ...
            if join_result.is_err() {
                error(&target!(), &format!("Error joining proxy channel to TCP stream IO copy task handle: err={:?}", join_result.as_ref().err().unwrap()));
            }
            ...
            if join_result.is_err() {
                error(&target!(), &format!("Error joining proxy TCP stream to channel IO copy task handle: err={:?}", join_result.as_ref().err().unwrap()));
            }
            if let Err(err) = join_result.unwrap() {
                match err {
                    AppError::StreamEOF => {}
    ...
    fn perform_shutdown(proxy_key: &str,
        ...
        match tcp_stream.shutdown(Shutdown::Both) {
            ...
            Err(err) => error(&target!(), &format!("Error shutting down proxy tcp stream: proxy_stream={}, err={:?}", &proxy_key, err))
        }
