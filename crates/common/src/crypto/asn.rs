...
pub fn stringify_asn_value(asn_attr: &Any<'_>) -> Result<String, AppError> {
    ...
    match asn_attr.header.tag() {
        Tag::Boolean => asn_attr
            .clone()
            .bool()
            .map(|v| v.to_string())
            .or_else(convert_err_fn),
        Tag::Enumerated => asn_attr
            .clone()
            .enumerated()
            .map(|v| v.0.to_string())
            .or_else(convert_err_fn),
        Tag::GeneralizedTime => asn_attr
            .clone()
            .generalizedtime()
            .map(|v| v.to_string())
            .or_else(convert_err_fn),
        Tag::GeneralString => asn_attr
            ...
            .map(|v| v.string())
            .or_else(convert_err_fn),
        Tag::Ia5String => asn_attr
            .clone()
            .ia5string()
            .map(|v| v.string())
            .or_else(convert_err_fn),
        Tag::Integer => asn_attr
            .clone()
            .integer()
            .map(|v| v.as_i64())
            .map(|v| v.unwrap().to_string())
            .or_else(convert_err_fn),
        Tag::OctetString => match asn_attr.clone().octetstring() {
            Ok(octet_str) => {
                Ok(octet_str
                    .as_ref()
                    .iter()
