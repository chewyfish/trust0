...
impl Device {
    ...
    pub fn new(device_cert_chain: &Vec<Certificate>) -> Result<Self, AppError> {
        ...
        let cert_subj = x509_cert.subject.iter()
            .flat_map(|dn| dn.iter())
            .map(|attr| {
                match asn::stringify_asn_value(attr.attr_value()) {
        ...
        if let Some(cert_alt_subj_ext) = x509_cert.subject_alternative_name()
            ...
            if let Some(uri_value) = cert_alt_subj.get("URI") {
                cert_access_context = serde_json::from_str(uri_value.get(0).unwrap().as_str())
                    .map_err(|err| AppError::GenWithMsgAndErr("Invalid Certificate Context JSON".to_string(), Box::new(err)))?;
            }
        }
...
mod tests {
    ...
    fn device_new_fn_when_invalid_client_cert() -> Result<(), AppError> {
        ...
        if let Ok(device) = &device_result {
            ...
            assert_eq!(device.cert_access_context.user_id, default_device.cert_access_context.user_id);
            assert_eq!(device.cert_access_context.platform, default_device.cert_access_context.platform);
            return Ok(());
        }
