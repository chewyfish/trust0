...
mod tests {
    ...
    const VALID_SERVICE_DB_FILE_PATHPARTS: [&str; 3] = [env!("CARGO_MANIFEST_DIR"), "testdata", "db-service.json"];
    const INVALID_SERVICE_DB_FILE_PATHPARTS: [&str; 3] = [env!("CARGO_MANIFEST_DIR"), "testdata", "db-service-INVALID.json"];
    ...
    fn inmemsvcrepo_connect_to_datasource_when_valid_filepath() {
        ...
        if let Err(err) = service_repo.connect_to_datasource(valid_service_db_pathstr) {
            panic!("Unexpected result: file={}, err={:?}", valid_service_db_pathstr, &err);
        }

        let expected_service_db_map: HashMap<u64, Service> = HashMap::from([
            (200, Service {service_id: 200, name:  "Service200".to_string(), transport: Transport::TCP, host: "localhost".to_string(), port:  8200}),
            (201, Service {service_id: 201, name:  "Service201".to_string(), transport: Transport::TCP, host: "localhost".to_string(), port:  8201}),
            (202, Service {service_id: 202, name:  "Service202".to_string(), transport: Transport::TCP, host: "localhost".to_string(), port:  8202}),
            (203, Service {service_id: 203, name:  "chat-tcp".to_string(), transport: Transport::TCP, host: "localhost".to_string(), port:  8500}),
            (204, Service {service_id: 204, name:  "echo-udp".to_string(), transport: Transport::UDP, host: "localhost".to_string(), port:  8600})
        ]);

        let actual_service_db_map: HashMap<u64, Service> = HashMap::from_iter(
            service_repo.services.into_inner().unwrap().iter().map(|e| (e.0.clone(), e.1.clone())).collect::<Vec<(u64, Service)>>());

        assert_eq!(actual_service_db_map.len(), expected_service_db_map.len());
        assert_eq!(actual_service_db_map.iter().filter(|entry| !expected_service_db_map.contains_key(entry.0)).count(), 0);
    }
    ...
    fn inmemsvcrepo_put() {
        ...
        let service_key = 1;
        let service = Service { service_id: 1, name: "svc1".to_string(), transport: Transport::TCP, host: "site1".to_string(), port: 100 };
    ...
    fn inmemsvcrepo_get_when_invalid_service() {
        ...
        service_repo.services.write().unwrap().insert(service_key, service);
    ...
    fn inmemsvcrepo_get_when_valid_service() {
        ...
        let services = [
            Service { service_id: 1, name: "svc1".to_string(), transport: Transport::TCP, host: "site1".to_string(), port: 100 },
            Service { service_id: 2, name: "svc2".to_string(), transport: Transport::TCP, host: "site2".to_string(), port: 200 },
            Service { service_id: 3, name: "svc3".to_string(), transport: Transport::UDP, host: "site3".to_string(), port: 300 }
        ];

        service_repo.services.write().unwrap().insert(service_keys[0], services[0].clone());
        service_repo.services.write().unwrap().insert(service_keys[1], services[1].clone());
        service_repo.services.write().unwrap().insert(service_keys[2], services[2].clone());
    ...
    fn inmemsvcrepo_get_all() {
        ...
        let expected_access_db_map: HashMap<u64, Service> = HashMap::from([
            (1, Service { service_id: 1, name: "svc1".to_string(), transport: Transport::TCP, host: "site1".to_string(), port: 100 }),
            (2, Service { service_id: 2, name: "svc2".to_string(), transport: Transport::TCP, host: "site2".to_string(), port: 200 }),
            (3, Service { service_id: 3, name: "svc3".to_string(), transport: Transport::UDP, host: "site3".to_string(), port: 300 })
        ]);


        assert_eq!(
            actual_services.iter()
                .filter(|entry| !expected_access_db_map.contains_key(&entry.service_id)).count(),
            0);
    }
    ...
    fn inmemsvcrepo_delete_when_valid_service() {
        ...
        service_repo.services.write().unwrap().insert(service_key, service.clone());
