...
mod tests {
    ...
    const VALID_ACCESS_DB_FILE_PATHPARTS: [&str; 3] = [env!("CARGO_MANIFEST_DIR"), "testdata", "db-access.json"];
    const INVALID_ACCESS_DB_FILE_PATHPARTS: [&str; 3] = [env!("CARGO_MANIFEST_DIR"), "testdata", "db-access-INVALID.json"];
    ...
    fn inmemaccessrepo_connect_to_datasource_when_valid_filepath() {
        ...
        if let Err(err) = access_repo.connect_to_datasource(valid_access_db_pathstr) {
            panic!("Unexpected result: file={}, err={:?}", valid_access_db_pathstr, &err);
        }

        let expected_access_db_map: HashMap<(u64, u64), ServiceAccess> = HashMap::from([
            ((100, 200), ServiceAccess { user_id: 100, service_id: 200 }),
            ((100, 203), ServiceAccess { user_id: 100, service_id: 203 }),
            ((100, 204), ServiceAccess { user_id: 100, service_id: 204 }),
            ((101, 202), ServiceAccess { user_id: 101, service_id: 202 }),
            ((101, 203), ServiceAccess { user_id: 101, service_id: 203 })
        ]);

        let actual_access_db_map: HashMap<(u64, u64), ServiceAccess> = HashMap::from_iter(
            access_repo.accesses.into_inner().unwrap().iter().map(|e| (e.0.clone(), e.1.clone())).collect::<Vec<((u64, u64), ServiceAccess)>>());

        assert_eq!(actual_access_db_map.len(), expected_access_db_map.len());
        assert_eq!(actual_access_db_map.iter().filter(|entry| !expected_access_db_map.contains_key(entry.0)).count(), 0);
    }
    ...
    fn inmemaccessrepo_put() {
        ...
        let access_key = (1,2);
        let access = ServiceAccess { user_id: 1, service_id: 2 };
    ...
    fn inmemaccessrepo_get_when_invalid_user() {
        ...
        access_repo.accesses.write().unwrap().insert(access_key, access);
    ...
    fn inmemaccessrepo_get_when_valid_user_and_service() {
        ...
        let accesses = [
            ServiceAccess { user_id: 1, service_id: 2},
            ServiceAccess { user_id: 3, service_id: 4}];

        access_repo.accesses.write().unwrap().insert(access_keys[0], accesses[0].clone());
        access_repo.accesses.write().unwrap().insert(access_keys[1], accesses[1].clone());
    ...
    fn inmemaccessrepo_get_all_for_user_when_invalid_user() {
        ...
        let accesses = [
            ServiceAccess { user_id: 1, service_id: 2},
            ServiceAccess { user_id: 3, service_id: 4},
            ServiceAccess { user_id: 1, service_id: 5}];

        access_repo.accesses.write().unwrap().insert(access_keys[0], accesses[0].clone());
        access_repo.accesses.write().unwrap().insert(access_keys[1], accesses[1].clone());
        access_repo.accesses.write().unwrap().insert(access_keys[2], accesses[2].clone());
    ...
    fn inmemaccessrepo_get_all_for_user_when_valid_user() {
        ...
        let expected_access_db_map: HashMap<(u64, u64), ServiceAccess> = HashMap::from([
            ((1, 2), ServiceAccess { user_id: 1, service_id: 2 }),
            ((1, 5), ServiceAccess { user_id: 1, service_id: 5 })
        ]);

        assert_eq!(
            actual_accesses.iter()
                .filter(|entry| !expected_access_db_map.contains_key(&(entry.user_id, entry.service_id))).count(),
            0);
    }
    ...
    fn inmemaccessrepo_delete_when_valid_user_and_service() {
        ...
        access_repo.accesses.write().unwrap().insert(access_key, access.clone());
