...
impl server_std::ServerVisitor for TcpClientProxyServerVisitor {
    ...
    fn on_conn_accepted(&mut self, connection: conn_std::Connection) -> Result<(), AppError> {
        ...
        let proxy_key = ProxyEvent::key_value(&ProxyType::TcpAndTcp, tcp_stream.peer_addr().ok(), tcp_stream.local_addr().ok());
        let client_stream = tcp_stream.try_clone().map_err(|err|
            AppError::GenWithMsgAndErr(format!("Unable to clone client stream: client_stream={:?}", &tcp_stream), Box::new(err)))?;
