...
pub trait ServiceMgr : Send {
    ...
    fn startup(&mut self, service: &Service, proxy_addrs: &ProxyAddrs)
...
impl ClientServiceMgr {
    ...
    fn process_next_proxy_event(service_mgr: &Arc<Mutex<dyn ServiceMgr>>, proxy_events_receiver: &Receiver<ProxyEvent>)
        ...
        match service.transport {
            ...
            Transport::TCP => {
                ...
                if ! self.testing_mode {
                    ...
                    let service_proxy_thread = thread::spawn(move || {
...
pub mod tests {
    ...
    fn clisvcmgr_process_next_proxy_event_when_ignorable_evt() {
        ...
        proxy_visitor.expect_remove_proxy_for_key().with(predicate::eq(proxy_key.clone())).never();

        let mut service_mgr = ClientServiceMgr::new(app_config, mpsc::channel().0, events_channel.0.clone());
        service_mgr.testing_mode = true;
        service_mgr.services_by_proxy_key.lock().unwrap().insert(proxy_key.clone(), proxy_svc_id);
        service_mgr.service_proxy_visitors.insert(proxy_svc_id, Arc::new(Mutex::new(proxy_visitor)));
        let service_mgr: Arc<Mutex<dyn ServiceMgr>> = Arc::new(Mutex::new(service_mgr));

        let msg = ProxyEvent::Message(proxy_key.clone(), SocketAddr::from_str("127.0.0.1:3000").unwrap(), "data".as_bytes().to_vec());
    ...
    fn clisvcmgr_process_next_proxy_event_when_closed_evt() {
        ...
        let mut proxy_visitor = MockCliSvcProxyVisitor::new();
        proxy_visitor.expect_remove_proxy_for_key().with(predicate::eq(proxy_key.clone())).times(1).return_once(|_| { true });
    ...
    fn clisvcmgr_startup_when_already_started() {
        ...
        let mut service_mgr = ClientServiceMgr::new(app_config, mpsc::channel().0, mpsc::channel().0);
        service_mgr.testing_mode = true;
        service_mgr.service_addrs.insert(service.service_id, proxy_addrs.clone());
        ...
        assert_eq!(service_mgr.service_proxy_visitors.len(), orig_svc_proxy_visitors_len);
    }
    ...
    fn clisvcmgr_start_when_tcp_service() {
        ...
        assert_eq!(service_mgr.service_proxy_visitors.len(), orig_svc_proxy_visitors_len + 1);
    }
    ...
    fn clisvcmgr_shutdown_when_2_service_proxies() {
        ...
        proxy_visitor200.expect_set_shutdown_requested().times(1).return_once(|| {()});
        proxy_visitor200.expect_shutdown_connections().times(1).return_once(|_| { Ok(()) });
        let mut proxy_visitor201 = MockCliSvcProxyVisitor::new();
        proxy_visitor201.expect_set_shutdown_requested().times(1).return_once(|| {()});
        proxy_visitor201.expect_shutdown_connections().times(1).return_once(|_| { Ok(()) });

        let mut service_mgr = ClientServiceMgr::new(app_config, mpsc::channel().0, mpsc::channel().0);
        service_mgr.testing_mode = true;
        service_mgr.service_proxy_visitors.insert(200, Arc::new(Mutex::new(proxy_visitor200)));
        service_mgr.service_proxy_visitors.insert(201, Arc::new(Mutex::new(proxy_visitor201)));
