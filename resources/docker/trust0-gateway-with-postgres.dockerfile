...
RUN cargo chef cook --release --recipe-path recipe.json --bin trust0-gateway --features postgres_db
COPY ./Cargo.toml ./Cargo.lock /app/
COPY ./crates /app/crates
RUN cargo build --release --bin trust0-gateway --features postgres_db
